...
from openke.module.modelIdx import LN_DistMult
...
trainer = Trainer(model = distmult, data_loader = train_dataloader, train_times = 10, alpha = 0.05, lr_decay = 0.1, use_gpu = True, cuda_idx = cuda_, on_step=True, opt_method = "adam")
tester = Tester(model = distmult, data_loader = test_dataloader, use_gpu = True, cuda_idx = cuda_, trainer = trainer)

for idx in range(1, 500):
	print("		#####", idx)
	# train the model
	trainer.run()
	distmult.save_checkpoint('./'+dt+'/distmult_idx'+str(dim)+'.ckpt')

# test the model
distmult.load_checkpoint('./'+dt+'/distmult_idx'+str(dim)+'.ckpt')
